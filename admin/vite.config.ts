        ...
        rollupOptions: {
            output: {
                manualChunks(id) {
                    ...
                    // 兼容 pnpm：取「最后一个 node_modules/ 之后」的真实包名，
                    // 否则会拿到 .pnpm 目录，生成以点开头的 .pnpm-xxx.js（被服务器/CDN 拦截 404）
                    const after = id.split('node_modules/').pop() || ''
                    let name = after.split('/')[0]
                    if (name.startsWith('@')) {
                        name = after.split('/').slice(0, 2).join('/')
                    }
                    // 清理前导点/@ 及路径分隔符，确保文件名不以点开头
                    return name.replace(/^[@._]+/, '').replace(/[\\/@]/g, '-') || 'vendor'
                }
            }
        }
    },
