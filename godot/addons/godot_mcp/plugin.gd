...
func _poll_clients() -> void:
	for c in _clients.duplicate():
		...
		while "\n" in _buffers[key]:
			var idx: int = _buffers[key].find("\n")
			var line: String = _buffers[key].substr(0, idx).strip_edges()
...
func _node_dict(node: Node, depth := 0) -> Dictionary:
	...
	if depth < 8:
		for c in node.get_children():
			children.append(_node_dict(c, depth + 1))
	return {
...
func _cmd_node_signals(p: Dictionary) -> Dictionary:
	...
	for s in node.get_signal_list():
		...
		sigs.append({"name": s["name"], "args": arg_names})
	var conns: Array = []
...
func _cmd_detect_circular_deps(_p: Dictionary) -> Dictionary:
	...
	for script_path in extends_map:
		var visited: Array = [script_path]; var current: String = script_path
...
func _cmd_compare_screenshots(p: Dictionary) -> Dictionary:
	...
	var sample_n := 2000
	var w: int = min(img.get_width(), _ref_screenshot.get_width())
	var h: int = min(img.get_height(), _ref_screenshot.get_height())
	var diff_count := 0
	for _i in range(sample_n):
		var x: int = randi() % w; var y: int = randi() % h
